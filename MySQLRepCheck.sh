...
#script to check Mysql replication is running
     ...
     primary=`mysql --user=emisuser --password=T3ster -e "show slave status\G" | grep -i " Master_Log_File:" | awk '{print $2}'`
    if [[ $primary != *"mysql-bin"* ]]
then
   exit 1
fi
    status=0
    MasterHost=`ssh emisuser@crm-mysql-rep.emis.local hostname`
    SlaveHost="$HOSTNAME"
    emails="Gary.Williams@emishealth.com Mark.Burn@emishealth.com Frazer.Bradburn@emishealth.com " #multiple emails space separated
    ...
    SQLresponse=`mysql --user=emisuser --password=T3ster  emis_sugarcrm -e "show slave status \G" |grep -i "Slave_SQL_Running"| awk '{print $2}'`
    IOresponse=`mysql  --user=emisuser --password=T3ster  emis_sugarcrm -e "show slave status \G" |grep -i "Slave_IO_Running" | awk '{print $2}'`
